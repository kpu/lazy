...
void AddHypothesis(ContextBase &context, Trie &root, const NBestComplete &end) {
  ...
  while (true) {
    if (left == state.left.length) {
      FindOrInsert(context, node, kCompleteAdd - state.left.full, state, left, true, right, false);
      for (; right < state.right.length; ++right) {
        FindOrInsert(context, node, state.right.words[right], state, left, true, right + 1, false);
      }
      break;
    }
    FindOrInsert(context, node, state.left.pointers[left], state, left + 1, false, right, false);
    left++;
    if (right == state.right.length) {
      FindOrInsert(context, node, kCompleteAdd - state.left.full, state, left, false, right, true);
      for (; left < state.left.length; ++left) {
        FindOrInsert(context, node, state.left.pointers[left], state, left + 1, false, right, true);
      }
      break;
    }
    FindOrInsert(context, node, state.right.words[right], state, left, false, right + 1, false);
    right++;
  }

  FindOrInsert(context, node, kCompleteAdd - state.left.full, state, state.left.length, true, state.right.length, true);
